...
{
	int err;
#if defined(CONFIG_ARCH_BITMAIN) || (CONFIG_ARCH_SOPHGO)
	const char *fpga_autoneg;
...
#if defined(CONFIG_ARCH_BITMAIN) || (CONFIG_ARCH_SOPHGO)
	np = of_find_node_by_name(of_root, "eth-fpga-config");
	if (np) {
		if (!of_property_read_string(np, "autoneg", &fpga_autoneg))
			dev_info(&phydev->mdio.dev, "current platform is fpga,autoneg is %s\n", fpga_autoneg);
		else
			fpga_autoneg = "enabled";
	} else {
		fpga_autoneg = "enabled";
	}

	if (!strcmp(fpga_autoneg, "disable")) {
		// FPGA only support 100MHz, so disable auto negotiation
		dev_info(&phydev->mdio.dev, "FPGA workaround!\n");
