...
static void dwc_dostart(struct dw_dma_chan *dwc, struct dw_desc *first)
{
	...
	channel_writeq(dwc, LLP, first->txd.phys | lms);
...
#ifdef CONFIG_ARCH_MANGO
	desc->txd.phys = phys - 0x8000000000 + 0x30000000000;
#else
	desc->txd.phys = phys;
#endif
	pr_debug("%s, desc->txd.phy(0x%llx)\n", __func__, desc->txd.phys);
	return desc;
}
...
#ifdef CONFIG_ARCH_MANGO
		child->txd.phys = child->txd.phys - 0x30000000000 + 0x8000000000;
#endif
		dma_pool_free(dw->desc_pool, child, child->txd.phys);
		dwc->descs_allocated--;
	}
#ifdef CONFIG_ARCH_MANGO
	desc->txd.phys = desc->txd.phys - 0x30000000000 + 0x8000000000;
#endif
	pr_debug("%s, desc->txd.phy(0x%llx)\n", __func__, desc->txd.phys);
...
static dma_addr_t fix_dma_bug_copy_get(struct dw_dma_chan *dwc,
	struct dma_chan *chan, struct scatterlist *sgl, int sg_index, int sg_len, int size)
{
	int i;
	dma_addr_t bug_addr;
...
static inline u32 dwc_get_sent(struct dw_dma_chan *dwc)
{
	...
	return (bts & DWC_BLOCK_TS_MASK) * (1 << (ctl >> 8 & 7));
}

static inline void dwc_chan_disable(struct dw_dma *dw, struct dw_dma_chan *dwc)
{
	dma_clear_bit(dw, CFG, 0x0001);
	dma_clear_bit(dw, CH_EN, dwc->mask);
	while (dma_readq(dw, CH_EN) & dwc->mask)
		cpu_relax();
	dma_set_bit(dw, CFG, 0x0001);
}
		...
		size_t len, unsigned long flags)
{
...
#ifdef CONFIG_ARCH_MANGO
	src = src - 0x8000000000 + 0x30000000000;
	dest = dest - 0x8000000000 + 0x30000000000;
#endif
	pr_debug("%s, from 0x%llx to 0x%llx, 0x%lx bytes\n",
	...
	for (offset = 0; offset < len; offset += xfer_count << trans_width) {
		...
		if (!first) {
			...
			lli_write(prev, llp, desc->txd.phys | DWC_LLP_LMS(m_master));
...
/* Change period to 128 bytes due to the maximum LLI data length of Bitmain sysDMA (32 bytes * 4 bytes bus width)*/
	...
	for (i = 0; i < periods; i++) {
		...
		if (last) {
			lli_write(last, llp, desc->txd.phys | DWC_LLP_LMS(dwc->dws.m_master));
	...
	lli_write(last, llp, cdesc->desc[0]->txd.phys | DWC_LLP_LMS(dwc->dws.m_master));
...
static void dwc_handle_cyclic(struct dw_dma *dw, struct dw_dma_chan *dwc)
{
	...
	if (first->txd.phys != DWC_LLP_LOC(llp)) {
		for (i = 0; i < dwc->cdesc->periods; i++) {
			...
			first->residue -= dwc_get_sent(dwc);
			if (desc->txd.phys == DWC_LLP_LOC(llp))
				break;
		}
	}

	dev_dbg(chan2dev(&dwc->chan), "new cyclic period llp 0x%08llx, SRC ADDR=0x%llx\n",
			channel_readq(dwc, LLP), channel_readq(dwc, SAR));
...
static ssize_t debug_signal_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return 0;
}
static DEVICE_ATTR_RO(debug_signal);

static struct attribute *debug_attrs[] = {
	&dev_attr_debug_signal.attr,
	NULL
};
...
static int dw_dma_probe(struct platform_device *pdev)
{
	...
	if (err) {
		dev_err(dev,
			"DesignWare DMA resource init error %d\n", err);
		goto err_resource;
	}
	...
	if (err) {
		dev_err(dev,
			"DesignWare DMA parse devicetree error %d\n", err);
		goto err_dt;
	}

	dw->clk = devm_clk_get(dw->dev, "tsdmaclk");
	if (!IS_ERR(dw->clk))
	...
	if (err) {
		dev_err(dev,
			"DesignWare DMA channel init error %d\n", err);
		goto err_chan;
	}
	...
	dev_info(dev, "DesignWare DMA Controller, %d channels, probe done!\n",
		 dw->nr_channels);
...
static const struct dev_pm_ops dw_dev_pm_ops = {
	SET_LATE_SYSTEM_SLEEP_PM_OPS(dw_dma_suspend_late, dw_dma_resume_early)
};
