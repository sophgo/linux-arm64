...
static int lm90_get_temp11(struct lm90_data *data, int index)
{
	...
	if (index == REMOTE_TEMP)
		temp = temp * data->gain + data->offset;
#endif
		       ...
		       struct i2c_board_info *info)
{
	...
	if (man_id == 0x55 && chip_id == 0x00 &&
		...
		if ((local_ext & 0x0F) == 0x00 &&
		    ...
		    (dfc & 0xfc) == 0x00) {
#ifdef CONFIG_ARCH_BITMAIN
			if ((address == 0x4c || address == 0x6b) && !(chen & 0x03))
#else
...
#ifdef CONFIG_ARCH_BITMAIN
static int bm_tmp451_probe(struct lm90_data *data)
{
	const char *str;
	int err;
	...
	data->gain = 1;
	data->offset = 0;
	err = of_property_read_string(dev_of_node(&data->client->dev),
				      "gain", &str);
	if (!err) {
		err = kstrtol(str, 0, &data->gain);
		if (err)
			return err;
	}
	err = of_property_read_string(dev_of_node(&data->client->dev),
...
#ifdef CONFIG_ARCH_BITMAIN
	err = bm_tmp451_probe(data);
	if (err) {
		dev_err(dev, "invalid gain or offset\n");
		return err;
	}
#endif
