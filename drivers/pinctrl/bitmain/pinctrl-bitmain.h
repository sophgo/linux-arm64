#ifndef __BM_PINCTRL_CORE_H__
#define __BM_PINCTRL_CORE_H__
...
struct bm_soc_pinctrl_data {
	const struct pinctrl_pin_desc *pins;
	unsigned int npins;
	struct bm_group *groups;
	int groups_count;
	const struct bm_pmx_func *functions;
	int functions_count;
	struct class *p_class;
};

int bitmain_pinctrl_probe(struct platform_device *pdev);
int bm_pmux_probe(struct platform_device *pdev);
