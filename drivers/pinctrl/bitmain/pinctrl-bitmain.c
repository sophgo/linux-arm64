...
static struct bm_soc_pinctrl_data *get_pinmux_data(struct pinctrl_dev *pctldev)
{
	struct bm_pinctrl *bmpctrl = pinctrl_dev_get_drvdata(pctldev);
...
{
	struct bm_soc_pinctrl_data *data = get_pinmux_data(pctldev);

	return data->functions_count;
}

static const char *bm_get_fname(struct pinctrl_dev *pctldev,
				...
				struct device_attribute *attr, char *buf)
{
	...
	struct bm_soc_pinctrl_data *data;

	bmpctrl = dev_get_drvdata(dev);
	data = (struct bm_soc_pinctrl_data *)bmpctrl->data;
				...
				const char *buf, size_t size)
{
	...
	group = selector/2;
	bm_set_mux(bmpctrl->pctl, selector, group);
...
int bitmain_pinctrl_probe(struct platform_device *pdev)
{
	struct bm_pinctrl *bmpctrl;
	...
	data = (struct bm_soc_pinctrl_data *)of_device_get_match_data(&pdev->dev);
	if (!data)
		return -EINVAL;
	...
	bmpctrl->syscon_pinctl = syscon;

	ret = device_property_read_u32(&pdev->dev,
		"top_pinctl_offset", &bmpctrl->top_pinctl_offset);
	if (ret < 0) {
	...
	bmpctrl->data = (void *)data;
	bmpctrl->pctl = devm_pinctrl_register(&pdev->dev, desc, bmpctrl);
	...
	pin_dev = device_create(data->p_class, &pdev->dev, MKDEV(0, 0), bmpctrl, "bm_pinmux");
	if (IS_ERR(pin_dev))
