 ...
 * BM1684X: 0x16860000
 */
uint32_t sophon_get_chip_id(void)
{
	u32 reg_val = 0;

	regmap_read(bm_top_dev.syscon, CHIP_ID_VERSION, &reg_val);

	return reg_val;
}
EXPORT_SYMBOL(sophon_get_chip_id);
...
static int bm_top_probe(struct platform_device *pdev)
{
	...
	bm_set_auto_clock_gating(0x3FFE);
