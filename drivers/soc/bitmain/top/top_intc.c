...
/*
 * here we assume all gic hwirq and tic hwirq should
 * be contiguous.
 * topc_intc hwirq is index of bitmap (both software and
 * hardware), and starts from 0.
 * so we use tic hwirq as index to get gic hwirq and its
 * irq data.
 * when used as a msi parent, tic hwirq is written to Top
 * reg for triggering irq by a PCIe device.
 *
 * now we pre-requested gic interrupt, but may try request
 * gic interrupt when needed, like gicp_irq_domain_alloc.
 */
struct top_intc_data {
	...
	irq_hw_number_t		gic_hwirqs[MAX_IRQ_NUMBER];
	int			gic_irqs[MAX_IRQ_NUMBER];
	struct irq_data		*gic_irq_datas[MAX_IRQ_NUMBER];
	int			tic_to_gic[MAX_IRQ_NUMBER]; // mapping from tic hwirq to gic hwirq
};
				...
				void *args)
{
	...
	if (data->for_msi) {
		...
		data->tic_to_gic[hwirq] = data->gic_hwirqs[hwirq];
	}

	pr_info("%s hwirq %ld, irq %d, gic irq %d, total %d\n", __func__,
		hwirq, virq, data->gic_irqs[hwirq], nr_irqs);
	return 0;
}
...
static void top_intc_ack_irq(struct irq_data *d)
{
	...
	pr_debug("%s %ld, parent %s/%ld\n", __func__, d->hwirq,
		gic_irq_data->domain->name, gic_irq_data->hwirq);
	if (gic_irq_data->chip->irq_ack)
		gic_irq_data->chip->irq_ack(gic_irq_data);
}
...
static void top_intc_mask_irq(struct irq_data *d)
{
	...
	pr_debug("%s %ld, parent %s/%ld\n", __func__, d->hwirq,
		gic_irq_data->domain->name, gic_irq_data->hwirq);
	gic_irq_data->chip->irq_mask(gic_irq_data);
}
...
static void top_intc_unmask_irq(struct irq_data *d)
{
	...
	pr_debug("%s %ld, parent %s/%ld\n", __func__, d->hwirq,
		gic_irq_data->domain->name, gic_irq_data->hwirq);
	gic_irq_data->chip->irq_unmask(gic_irq_data);
}
...
static void top_intc_irq_handler(struct irq_desc *gic_desc)
{
	struct irq_chip *gic_chip = irq_desc_get_chip(gic_desc);
	struct top_intc_data *data = irq_desc_get_handler_data(gic_desc);
	irq_hw_number_t gic_hwirq = irq_desc_get_irq_data(gic_desc)->hwirq;
	irq_hw_number_t top_intc_hwirq;
	int top_intc_irq, i, ret;

	chained_irq_enter(gic_chip, gic_desc);

	for (i = 0; i < data->irq_num; i++) {
		if (data->tic_to_gic[i] == gic_hwirq)
			break;
	}
	if (i < data->irq_num) {
		top_intc_hwirq = i;
		top_intc_irq = irq_find_mapping(data->domain, top_intc_hwirq);
		pr_debug("%s gic hwirq %ld, tic hwirq %ld, tic irq %d\n", __func__,
				gic_hwirq, top_intc_hwirq, top_intc_irq);
		if (top_intc_irq)
			ret = generic_handle_irq(top_intc_irq);
		pr_debug("%s handled tic irq %d, %d\n", __func__, top_intc_irq, ret);
	} else {
		pr_debug("%s not found tic hwirq for gic hwirq %ld\n", __func__, gic_hwirq);
		// workaround, ack unexpected(unregistered) interrupt
		writel(1 << (gic_hwirq - data->gic_hwirqs[0]), data->reg_clr);
	}

	chained_irq_exit(gic_chip, gic_desc);
}
...
static int top_intc_probe(struct platform_device *pdev)
{
	...
	// get irq numbers
	for (i = 0; i < ARRAY_SIZE(data->gic_hwirqs); i++) {
		char name[8];
		int irq;
		...
		data->gic_irqs[i] = irq;
		data->gic_irq_datas[i] = irq_get_irq_data(irq);
		data->gic_hwirqs[i] = data->gic_irq_datas[i]->hwirq;
		dev_info(&pdev->dev, "%s: gic hwirq %ld, gic irq %d\n", name,
				data->gic_hwirqs[i], data->gic_irqs[i]);
	}
	data->irq_num = i;
	dev_info(&pdev->dev, "got %d gic irqs\n", data->irq_num);
	...
	for (i = 1; i < data->irq_num; i++)
		irq_set_chained_handler_and_data(data->gic_irqs[i],
