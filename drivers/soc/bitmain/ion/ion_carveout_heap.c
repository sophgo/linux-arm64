...
static size_t gen_pool_max_chunk(struct gen_pool *pool)
{
	struct gen_pool_chunk *chunk;
	size_t avail_max = 0, avail = 0;
...
static size_t ion_carveout_heap_max(struct ion_heap *h)
{
	struct ion_carveout_heap *carveout_heap;

	if (!h || h->type != ION_HEAP_TYPE_CARVEOUT)
		return 0;

	carveout_heap = container_of(h, struct ion_carveout_heap, heap);
	if (carveout_heap)
		return gen_pool_max_chunk(carveout_heap->pool);
	else
		return 0;
}
...
static struct ion_heap_ops carveout_heap_ops = {
	...
	.max_chunk = ion_carveout_heap_max,
};
