...
static struct ion_of_heap bm_ion_heap_list[] = {
	...
	PLATFORM_HEAP("bitmain,carveout_vpu", 0,
...
long bitmain_ion_ioctl(struct ion_device *dev, unsigned int cmd, unsigned long arg)
{
	...
	case ION_IOC_BITMAIN_FLUSH_RANGE:
	{
		...
		if (IS_ERR(data.start)) {
			pr_err("ion flush failed, addr %p, size %zu!\n",
				data.start, data.size);
			return -EFAULT;
		}
		pr_debug("ion flush:0x%lx, size %zu\n", (unsigned long)data.start, data.size);

		/*
		 * when flush is needed, all pages should been written by user space already,
		 * so no page fault is expected here.
		 */
	...
	case ION_IOC_BITMAIN_INVALIDATE_RANGE:
	{
		...
		if (IS_ERR(data.start)) {
			pr_err("ion invalidate failed, addr %p, size %zu!\n",
				data.start, data.size);
			return -EFAULT;
		}
		...
		va = phys_to_virt(pa);
		pr_debug("ion invalidate:0x%lx/0x%lx, size %zu\n", (unsigned long)data.start, pa, data.size);

		/*
		 * __inval_dcache_area works well on user space virtual address, but it can't handle
		 * page fault, you'll get an "access to user memory outside uaccess routines" error.
		 * copy routines from copy_from_user does not help, don't know why. so keep this user
		 * VA to kernel VA conversion here. better to let use space pass down PA to us.
		 */
...
static int rmem_ion_device_init(struct reserved_mem *rmem, struct device *dev)
{
	...
	heap->base = rmem->base;
	heap->size = rmem->size;
	dev_info(dev, "%s: heap %s base %pa size %pa\n", __func__,
