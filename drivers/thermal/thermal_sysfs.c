...
static ssize_t
critical_action_show(struct device *dev, struct device_attribute *devattr,
		     char *buf)
{
	struct thermal_zone_device *tz = to_thermal_zone(dev);

	if (tz->tzp->critical_action == CRITICAL_ACTION_REBOOT)
...
static ssize_t
critical_action_store(struct device *dev, struct device_attribute *attr,
		      const char *buf, size_t count)
{
	struct thermal_zone_device *tz = to_thermal_zone(dev);

	if (!strcmp(buf, "reboot\n"))
		tz->tzp->critical_action = CRITICAL_ACTION_REBOOT;
	else
		tz->tzp->critical_action = CRITICAL_ACTION_POWEROFF;
