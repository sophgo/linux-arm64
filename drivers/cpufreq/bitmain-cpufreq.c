...
static int bm_cpufreq_verify(struct cpufreq_policy_data *policy)
{
...
static unsigned int bm_cpufreq_get_freq(unsigned int cpu)
{
	...
	freq = clk_get_rate(__clk_lookup("clk_div_a53_1")) / 1000;
	if (!freq) {
...
static int bm_cpufreq_init(struct cpufreq_policy *policy)
{
	int cur_freq;
	int ret = 0;
	struct cpufreq_frequency_table *table = per_cpu(freq_table, policy->cpu);
	...
	cur_freq = bm_cpufreq_get_freq(policy->cpu);

	pr_debug("cpufreq: cpu%d init at %u switching to %u\n",
			policy->cpu, policy->cur, cur_freq);
	policy->cur = cur_freq;
