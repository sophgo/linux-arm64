	...
	)
{
	...
	dbg_msg(TRACE_LEVEL_INFO, DBG_PNP, "kill in-flight requests now");
	...
	while (!list_empty(busy_list)) {
		...
		dbg_msg(TRACE_LEVEL_ERROR, DBG_PNP, "start kill urb*********");
		usb_kill_urb(render_ctx->main_urb);
		usb_kill_urb(render_ctx->zero_length_urb);
		dbg_msg(TRACE_LEVEL_ERROR, DBG_PNP, "end kill urb**********");
		delay_ms += 10;
		DELAY_MS(10);
		if (delay_ms > 500) {
			dbg_msg(TRACE_LEVEL_ERROR, DBG_PNP, "delay_ms > 500ms manual delete list_entry!*********");
			/* remove this render_ctx from busy_list*/
			spin_lock_bh(&dev_ctx->render.busy_list_lock);
			list_del(&render_ctx->list_entry);
			spin_unlock_bh(&dev_ctx->render.busy_list_lock);
	...
	dbg_msg(TRACE_LEVEL_INFO, DBG_PNP, "all in-flight requests killed");
}
