	...
	bool edid_connected)
{
	...
	//
	if (atomic_read(&dev_ctx->release_used) > 0)
		dbg_msg(TRACE_LEVEL_WARNING, DBG_PNP, "release process stop hmdi plugin callback");
	else{
		atomic_set(&dev_ctx->release_used, 1);

		if (dev_ctx->card_name == CARD_NAME_FL2000DX)
			fl2000_reg_bit_clear(dev_ctx, REG_OFFSET_0078, 17);

		// Disable U1U2
		dbg_msg(TRACE_LEVEL_WARNING, DBG_PNP,
			"fl2000_dongle_u1u2_setup.");
		...
		// Get EDID table.
		dbg_msg(TRACE_LEVEL_WARNING, DBG_PNP,
			"fl2000_monitor_read_edid.");
		fl2000_monitor_read_edid(dev_ctx);

		dbg_msg(TRACE_LEVEL_INFO, DBG_PNP,
			"Notify system to add monitor. %u", atomic_read(&dev_ctx->release_used));

		struct display_mode display_mode;

		memset(&display_mode, 0, sizeof(display_mode));
		display_mode.width = dev_ctx->render.display_mode.width;
		display_mode.height = dev_ctx->render.display_mode.height;
		display_mode.refresh_rate = dev_ctx->render.display_mode.refresh_rate;
		display_mode.input_color_format = dev_ctx->render.display_mode.input_color_format;
		display_mode.output_color_format = dev_ctx->render.display_mode.output_color_format;
		display_mode.use_compression = dev_ctx->render.display_mode.use_compression;
		display_mode.compress_size_limit = dev_ctx->render.display_mode.compress_size_limit;
		...
		if (waitqueue_active(&dev_ctx->ioctl_wait_q)) /* waitqueue_active wake up any sleeping process*/
		...
		atomic_set(&dev_ctx->release_used, 0);
	}
    //restart render for plugin hdmi
	dbg_msg(TRACE_LEVEL_VERBOSE, DBG_PNP, "<<<<");
}
