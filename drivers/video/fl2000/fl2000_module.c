							 ...
							 struct device_attribute *attr, char *buf)
{
	...
	struct dev_ctx *dev_ctx = usb_get_intfdata(iface);

	return sprintf(buf, "%d\n", dev_ctx->monitor_plugged_in);
}
static DEVICE_ATTR(status, 0644, __show_status, NULL);
...
static struct attribute_group fl2000_group_attrs = {
	.name = "hdmi_status",
	.attrs = dbg_attrs
	...
	const struct usb_device_id* usb_device_id)
{
	...
	case FL2000_IFC_INTERRUPT:
		...
		err = sysfs_create_group(&dev->kobj, &fl2000_group_attrs);
		if (err) {
			dbg_msg(TRACE_LEVEL_ERROR, DBG_PNP,
					"fl2000 sysfs_create_group failed?.");
			goto exit;
		}
...
fl2000_disconnect(struct usb_interface *ifc)
{
	...
	case FL2000_IFC_STREAMING:
		...
		break;
	case FL2000_IFC_INTERRUPT:
		sysfs_remove_group(&dev->kobj, &fl2000_group_attrs);
