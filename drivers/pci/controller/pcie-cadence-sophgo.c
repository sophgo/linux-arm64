...
static int cdns_pcie_slv_location_mapping(struct cdns_pcie_rc *rc)
{
	struct cdns_pcie *pcie = &rc->pcie;
	u32 apb_base = 0;
...
#ifndef MSI_TOP_INTC_MODE
static int cdns_pcie_msi_init(struct cdns_pcie_rc *rc)
{
	struct device *dev = rc->dev;
	struct cdns_pcie *pcie = &rc->pcie;
	...
	rc->msi_page = dma_alloc_coherent(dev, 1024, &rc->msi_data, (GFP_KERNEL|GFP_DMA32|__GFP_ZERO));
	if (rc->msi_page == NULL)
		return -1;

	dev_info(dev, "%s: msi_page is 0x%p, msi_data is 0x%llx\n", __func__, rc->msi_page, rc->msi_data);
...
static int cdns_pcie_host_init(struct device *dev, struct list_head *resources, struct cdns_pcie_rc *rc)
{
...
#ifdef MSI_TOP_INTC_MODE
extern struct irq_domain *cdns_pcie_get_parent_irq_domain(void);

static int cdns_pcie_msi_setup(struct cdns_pcie_rc *rc)
{
...
/* MSI int handler */
irqreturn_t cdns_handle_msi_irq(struct cdns_pcie_rc *rc)
{
	u32 i, pos, irq;
	unsigned long val;
...
{
	struct cdns_pcie_rc *rc = irq_data_get_irq_chip_data(d);
	u64 msi_target;
...
static int cdns_pcie_irq_domain_alloc(struct irq_domain *domain,
				    ...
				    void *args)
{
	struct cdns_pcie_rc *rc = domain->host_data;
	unsigned long flags;
	u32 i;
	int bit;
...
int cdns_pcie_allocate_domains(struct cdns_pcie_rc *rc)
{
	struct fwnode_handle *fwnode = of_node_to_fwnode(rc->dev->of_node);
...
void cdns_pcie_free_msi(struct cdns_pcie_rc *rc)
{
	if (rc->msi_irq) {
