...
static void smi_crtc_load_lut(struct drm_crtc *crtc)
{
	...
	u16 *r, *g, *b;
	int i , max_index, ctrl_index, dst_ctrl;
	ctrl_index = 0;
	dst_ctrl = 0;
	...
	for (i = 0; i < crtc->gamma_size; i++) {
		...
		if (g_specId == SPC_SM768)
			hw768_load_lut(dst_ctrl, offset, rgb);
	}


		if (g_specId == SPC_SM768)
			hw768_enable_gamma(dst_ctrl,1);
...
#if 0
static int smi_crtc_gamma_set(struct drm_crtc *crtc, u16 *red, u16 *green,
				u16 *blue, uint32_t size,
				struct drm_modeset_acquire_ctx *ctx)
{
	smi_crtc_load_lut(crtc);
				 ...
				 struct drm_display_mode *mode)
{
	...
	if (vrefresh > 61 || (vrefresh > 31 && vrefresh < 59))  //We only support and use 30Hz or 60Hz mode currently.
		return MODE_NOMODE;

	if((mode->hdisplay == 3840) && (vrefresh > 31))  //only support 4K@30Hz
		return MODE_NOMODE;
