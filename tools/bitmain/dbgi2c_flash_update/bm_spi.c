...
static unsigned long long ctrl_base;
...
static int spi_data_in_tran(unsigned long long spi_base, uint8_t *dst_buf,
	uint8_t *cmd_buf, int with_cmd, int addr_bytes, int data_bytes)
{
	...
	/* init tran_csr */
	tran_csr = dbgi2c_read32(spi_base + REG_SPI_TRAN_CSR);
	...
	dbgi2c_write32(spi_base + REG_SPI_FIFO_PT, 0); // flush FIFO before filling fifo
	if (with_cmd) {
		for (i = 0; i < ((cmd_bytes - 1) / 4 + 1); i++)
			dbgi2c_write32(spi_base + REG_SPI_FIFO_PORT, p_data[i]);
	}

	/* issue tran */
	dbgi2c_write32(spi_base + REG_SPI_INT_STS, 0); // clear all int
	dbgi2c_write32(spi_base + REG_SPI_TRAN_NUM, data_bytes);
	tran_csr |= BIT_SPI_TRAN_CSR_GO_BUSY;
	dbgi2c_write32(spi_base + REG_SPI_TRAN_CSR, tran_csr);

	/* check rd int to make sure data out done and in data started */
	while ((dbgi2c_read32(spi_base + REG_SPI_INT_STS) & BIT_SPI_INT_RD_FIFO) == 0)
		;
	...
	while (off < data_bytes) {
		...
		while ((dbgi2c_read32(spi_base + REG_SPI_FIFO_PT) & 0xF) != xfer_size)
			;
		for (i = 0; i < ((xfer_size - 1) / 4 + 1); i++)
			p_data[off / 4 + i] = dbgi2c_read32(spi_base + REG_SPI_FIFO_PORT);
		off += xfer_size;
	}

	/* wait tran done */
	while ((dbgi2c_read32(spi_base + REG_SPI_INT_STS) & BIT_SPI_INT_TRAN_DONE) == 0)
		;
	dbgi2c_write32(spi_base + REG_SPI_FIFO_PT, 0);    //should flush FIFO after tran
	return 0;
}

static int spi_data_read(unsigned long long spi_base, uint8_t *dst_buf, int addr, int size)
{
	uint8_t cmd_buf[4];
...
void bm_spi_flash_read_sector(unsigned long long spi_base, uint32_t addr, uint8_t *buf)
{
...
void bm_spi_init(unsigned long long base)
{
	uint32_t tran_csr = 0;
	...
	dbgi2c_write32(ctrl_base + REG_SPI_DMMR, 0);
	// soft reset
	dbgi2c_write32(ctrl_base + REG_SPI_CTRL, dbgi2c_read32(ctrl_base + REG_SPI_CTRL) | BIT_SPI_CTRL_SRST | 0x3);
	// hardware CE control, soft reset won't change this reg...
	dbgi2c_write32(ctrl_base + REG_SPI_CE_CTRL, 0x0);
	...
	dbgi2c_write32(ctrl_base + REG_SPI_TRAN_CSR, tran_csr);
}

/* here are APIs for SPI flash programming */

static uint8_t spi_non_data_tran(unsigned long long spi_base, uint8_t *cmd_buf,
	uint32_t with_cmd, uint32_t addr_bytes)
{
	...
	dbgi2c_write32(spi_base + REG_SPI_FIFO_PT, 0); //do flush FIFO before filling fifo

	dbgi2c_write32(spi_base + REG_SPI_FIFO_PORT, p_data[0]);
...
static uint8_t spi_data_out_tran(unsigned long long spi_base, uint8_t *src_buf, uint8_t *cmd_buf,
	uint32_t with_cmd, uint32_t addr_bytes, uint32_t data_bytes)
{
	...
	tran_csr |= BIT_SPI_TRAN_CSR_GO_BUSY;
	dbgi2c_write32(spi_base + REG_SPI_TRAN_CSR, tran_csr);
	while ((dbgi2c_read32(spi_base + REG_SPI_FIFO_PT) & 0xF) != 0)
		; //wait for cmd issued
	...
	while (off < data_bytes) {
		...
		for (i = 0; i < ((xfer_size - 1) / 4 + 1); i++)
			dbgi2c_write32(spi_base + REG_SPI_FIFO_PORT, p_data[off / 4 + i]);
 ...
 */
static uint8_t spi_in_out_tran(unsigned long long spi_base, uint8_t *dst_buf, uint8_t *src_buf,
	uint32_t with_cmd, uint32_t addr_bytes, uint32_t send_bytes, uint32_t get_bytes)
{
	...
	for (i = 0; i < total_out_bytes; i++)
		dbgi2c_write8(spi_base + REG_SPI_FIFO_PORT, p_data[i]);

	/* issue tran */
	dbgi2c_write32(spi_base + REG_SPI_INT_STS, 0); //clear all int
	dbgi2c_write32(spi_base + REG_SPI_TRAN_NUM, get_bytes);
	tran_csr |= BIT_SPI_TRAN_CSR_GO_BUSY;
	dbgi2c_write32(spi_base + REG_SPI_TRAN_CSR, tran_csr);
	...
	for (i = 0; i < get_bytes; i++)
		p_data[i] = dbgi2c_read8(spi_base + REG_SPI_FIFO_PORT);

	dbgi2c_write32(spi_base + REG_SPI_FIFO_PT, 0); //should flush FIFO after tran
	return 0;
}

static uint8_t spi_write_en(unsigned long long spi_base)
{
	uint8_t cmd_buf[4];
...
static uint8_t spi_read_status(unsigned long long spi_base)
{
	uint8_t cmd_buf[4];
	uint8_t data_buf[4];
...
static uint8_t spi_read_status_high(unsigned long long spi_base)
{
	uint8_t cmd_buf[4];
	uint8_t data_buf[4];
...
static void spi_write_status(unsigned long long spi_base, uint16_t value)
{
	uint8_t cmd_buf[4];
	uint8_t data_buf[4];
...
uint32_t bm_spi_read_id(unsigned long long spi_base)
{
	uint8_t cmd_buf[4];
	uint8_t data_buf[4];
	uint32_t read_id = 0;
...
static uint8_t spi_page_program(unsigned long long spi_base, uint8_t *src_buf, uint32_t addr, uint32_t size)
{
	uint8_t cmd_buf[4];
...
static void spi_sector_erase(unsigned long long spi_base, uint32_t addr)
{
	uint8_t cmd_buf[4];
...
int bm_spi_flash_erase_sector(unsigned long long spi_base, uint32_t addr)
{
...
int bm_spi_flash_program_sector(unsigned long long spi_base, uint32_t addr)
{
...
static int do_page_program(unsigned long long spi_base, uint8_t *src_buf, uint32_t addr, uint32_t size)
{
	uint8_t spi_status;
	uint32_t wait = 0;
...
static void fw_sp_read_test(unsigned long long spi_base, uint32_t addr)
{
